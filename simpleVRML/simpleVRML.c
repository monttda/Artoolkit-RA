...
static void mainLoop(void)
{
	...
	if ((image = arVideoGetImage()) != NULL) {
		...
		for (i = 0; i < gObjectDataCount; i++) {
			...
			if( (i%2 != 0) && gObjectData[i-1].visible && gObjectData[i].visible){
...
static void Display(void)
{
	...
	for (i = 0; i < gObjectDataCount; i++) {
		// Here we will make the if to check if both marks are on
		if ( ((gObjectData[i].visible != 0) && (gObjectData[i].vrml_id >= 0)) || 
			 ((gObjectData[i].visible != 0) && (gObjectData[i].collide_vrml_id >= 0)) ) {
